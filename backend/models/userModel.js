...
User.create = (nombre, email, password) => {
  return new Promise((resolve, reject) => { // El metodo regresa una promesa
    ...
    const query = 'INSERT INTO usuarios (nombre, email, password) VALUES (?, ?, ?)'; // creamos el usuario con una consulta mysql
    connection.query(query, [nombre, email, hashedPassword], (err, results) => { // hacemos la consulta y creacion en la conexion de a la base
